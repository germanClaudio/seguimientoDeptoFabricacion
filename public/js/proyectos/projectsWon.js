...
function sortTable(columnName) {
    const table = document.getElementById('projectWonTable');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('projectWonTable').style.display = 'none';
});
...
socket.on('projectsAllWon', (arrayProjects, arrUsers) => {
    ...
    let index = arrUsers.findIndex(el=> el.name == name.trim())
        ...
        if(index !== -1) {
            ...
            user ? renderProjectsWonForAdmin(arrayProjects) : renderProjectsWonForUser(arrayProjects)
        }   
})

// Función para generar los controles de paginación
const generarControlesPaginacion = () => {
    const totalPages = Math.ceil(proyectosWonGlobales.length / itemsPerPage);
    ...
    paginationHTML += `
        <button class="pagination-btn ${currentPage === 1 ? 'disabled' : ''}" 
            onclick="if(!this.classList.contains('disabled')) renderProjectsWonForAdmin(proyectosWonGlobales, ${currentPage - 1}, 'right')">
            &laquo; Anterior
        </button>`;
    ...
    if (currentPage > maxVisiblePages + 1) {
        paginationHTML += `
            <button class="pagination-btn" onclick="renderProjectsWonForAdmin(proyectosWonGlobales, 1)">1</button>
    ...
    for (let i = start; i <= end; i++) {
        paginationHTML += `
            <button class="pagination-btn ${i === currentPage ? 'active' : ''}" 
                onclick="renderProjectsWonForAdmin(proyectosWonGlobales, ${i})">
                ${i}
            </button>`;
    }
    ...
    if (currentPage < totalPages - maxVisiblePages) {
        paginationHTML += `
            <span class="pagination-ellipsis">...</span>
            <button class="pagination-btn" onclick="renderProjectsWonForAdmin(proyectosWonGlobales, ${totalPages})">${totalPages}</button>`;
    }

    // Botón Siguiente
    paginationHTML += `
        <button class="pagination-btn ${currentPage === totalPages ? 'disabled' : ''}" 
            onclick="if(!this.classList.contains('disabled')) renderProjectsWonForAdmin(proyectosWonGlobales, ${currentPage + 1}, 'left')">
            Siguiente &raquo;
        </button>`;
...
const cambiarItemsPorPagina = (nuevoItemsPerPage) => {
    ...
    renderProjectsWonForAdmin(proyectosWonGlobales);
};
...
const agregarSelectItemsPorPagina = () => {
    ...
    let selectHTML = `
        ...
        <select class="form-select small w-auto mx-2" id="itemsPerPageSelect" onchange="cambiarItemsPorPagina(parseInt(this.value))">
            ...
            <option value="${proyectosWonGlobales.length}" ${selectedItemsPerPage === proyectosWonGlobales.length ? 'selected' : ''}>Todos</option>
        </select>
        proyectos de ${proyectosWonGlobales.length}
    `;
...
const renderProjectsWonForAdmin = async (arrayProjects, page = 1, direction = 'none') => {
    proyectosWonGlobales = arrayProjects;
    currentPage = page;

    const container = document.getElementById('mostrarProyectosGanados');
    ...
    if (paginatedItems.length > 0) {
        html = paginatedItems.map((element) => {
            ...
            let text = "Cotizado", colorResult = grey, colorLevel
    ...
    document.getElementById('projectWonTable').style.display = 'block';
...
const generarControlesPaginacionUser = () => {
    ...
    paginationHTML += `
        <button class="pagination-btn ${currentPage === 1 ? 'disabled' : ''}" 
            onclick="if(!this.classList.contains('disabled')) renderProjectsWonForUser(proyectosWonGlobales, ${currentPage - 1}, 'right')">
            &laquo; Anterior
        </button>`;
    ...
    if (currentPage > maxVisiblePages + 1) {
        paginationHTML += `
            <button class="pagination-btn" onclick="renderProjectsWonForUser(proyectosWonGlobales, 1)">1</button>
            <span class="pagination-ellipsis">...</span>`;
    }
    ...
    for (let i = start; i <= end; i++) {
        paginationHTML += `
            <button class="pagination-btn ${i === currentPage ? 'active' : ''}" 
                onclick="renderProjectsWonForUser(proyectosWonGlobales, ${i})">
                ${i}
            </button>`;
    }
    ...
    if (currentPage < totalPages - maxVisiblePages) {
        paginationHTML += `
            <span class="pagination-ellipsis">...</span>
            <button class="pagination-btn" onclick="renderProjectsWonForUser(proyectosWonGlobales, ${totalPages})">${totalPages}</button>`;
    }

    // Botón Siguiente
    paginationHTML += `
        <button class="pagination-btn ${currentPage === totalPages ? 'disabled' : ''}" 
            onclick="if(!this.classList.contains('disabled')) renderProjectsWonForUser(proyectosWonGlobales, ${currentPage + 1}, 'left')">
            Siguiente &raquo;
        </button>`;
...
const cambiarItemsPorPaginaUser = (nuevoItemsPerPage) => {
    ...
    itemsPerPage = nuevoItemsPerPage;
    currentPage = 1; // Reiniciar a la primera página
    renderProjectsWonForUser(proyectosWonGlobales);
};
...
const renderProjectsWonForUser = async (arrayProjects, page = 1, direction = 'none') => {
    ...
    if (paginatedItems.length > 0) {
        html = paginatedItems.map((element) => {
            ...
            if(element.project[0].visible) {
                ...
                element.uNegocio === 'lineas'
                    ? btnGroup = `<a href="/api/clientes/${element.client[0]._id}" class="btn btn-secondary btn-sm my-2 shadow-lg" data-toggle="tooltip" data-placement="top" title="Ver proyecto"><i class="fa-solid fa-eye"></i></a>
                                    <a href="/api/ingenieriaLineas/selectWonProjects/${element.project[0]._id}" class="btn btn-primary btn-sm my-2 shadow-lg" title="Editar datos de Ingeniería Líneas"><i class="fa-solid fa-pencil"></i></a>
                                    <a href="/api/ajustes/selectLineasProjects/${element.project[0]._id}" class="btn btn-warning btn-sm my-2 shadow-lg" title="Editar datos de Fabricación"><i class="fa-solid fa-file-pen"></i></a>`
